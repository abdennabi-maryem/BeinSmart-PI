 ...
 * @author essia
 */
public class RugbyNewsController implements Initializable {

    @FXML
    private JFXListView<News> listViewRUGBY;
    @FXML
    private VBox vBoxRUGBY;
    ...
    public void AffichList() {
        ...
        vBoxRUGBY.setPadding(new Insets(10));
        vBoxRUGBY.setAlignment(Pos.CENTER);

        ObservableList<News> NewsList = sn.AfficherNews();
                                  listViewRUGBY.setStyle("  -fx-selection-bar: #90ee90 ; -fx-border-color:#90ee90");

        listViewRUGBY.setCellFactory((Callback<ListView<News>, ListCell<News>>) param -> {
        ...
        listViewRUGBY.setItems(sn.AfficherNewsParTypeSport("RUGBY"));

        vBoxRUGBY.getChildren().add(listViewRUGBY);

     
    }

    @FXML
    private void BACKRUGBYNEWS(ActionEvent event) {
             Parent root = null;
        try { 
    ...
    private void Details(MouseEvent event) {
         if (event.getClickCount() == 2 ){
             Parent root = null;
 News n=listViewRUGBY.getSelectionModel().getSelectedItem();
