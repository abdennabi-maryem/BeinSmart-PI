 ...
 * @author essia
 */
public class VolleyNewsController implements Initializable {

    @FXML
    private JFXListView<News> listViewVOLLEY;
    @FXML
    private VBox vBoxVOLLEY;
...
AffichList();    }    
    public void AffichList() {
        ...
        ObservableList<News> NewsList = sn.AfficherNews();
                                  listViewVOLLEY.setStyle("  -fx-selection-bar: #90ee90 ; -fx-border-color:#90ee90");

        listViewVOLLEY.setCellFactory((Callback<ListView<News>, ListCell<News>>) param -> {
...
Hbx.setStyle("-fx-background-color:  #ffffff00; ");
        ...
        listViewVOLLEY.setItems(sn.AfficherNewsParTypeSport("VOLLEYBALL"));

        vBoxVOLLEY.getChildren().add(listViewVOLLEY);
    ...
    @FXML
    private void BACKVOLLEYNEWS(ActionEvent event) {
            Parent root = null;
        try { 
    ...
    private void Details(MouseEvent event) {
        if (event.getClickCount() == 2 ){
             Parent root = null;
 News n=listViewVOLLEY.getSelectionModel().getSelectedItem();
