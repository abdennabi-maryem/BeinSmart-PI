 ...
 * @author essia
 */
public class HandNewsController implements Initializable {

    @FXML
    private JFXListView<News> listViewHAND;
    @FXML
    private VBox vBoxHAND;
...
public void AffichList() {
        ...
        vBoxHAND.setPadding(new Insets(10));
        vBoxHAND.setAlignment(Pos.CENTER);

        ObservableList<News> NewsList = sn.AfficherNews();
                                  listViewHAND.setStyle("  -fx-selection-bar: #90ee90 ; -fx-border-color:#90ee90");

        listViewHAND.setCellFactory((Callback<ListView<News>, ListCell<News>>) param -> {
...
Hbx.setStyle("-fx-background-color:  #ffffff00; ");
        ...
        listViewHAND.setItems(sn.AfficherNewsParTypeSport("HANDBALL"));

        vBoxHAND.getChildren().add(listViewHAND);

     
    }
    @FXML
    private void BACKHANDNEWS(ActionEvent event) {
             Parent root = null;
        try { 
    ...
    private void Details(MouseEvent event) {
             ...
             Parent root = null;
 News n=listViewHAND.getSelectionModel().getSelectedItem();
